...
class ECA_Module(nn.Module):
    ...
    Sử dụng 1D Conv để học mối tương quan giữa các kênh mà không giảm chiều.
    Rất nhẹ, chỉ thêm vài tham số nhưng tăng độ tập trung của feature map.
    """
    def __init__(self, channels, b=1, gamma=2):
        super(ECA_Module, self).__init__()
        # Tính toán kernel_size thích ứng dựa trên số kênh
...
class DSConv(nn.Module):
    ...
    Thay thế Conv 3x3 chuẩn trong DownSampling để giảm mạnh FLOPs và Params.
    """
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=1):
        ...
        # Bỏ qua BN/Acti ở đây vì khối DownSamplingBlock gốc sẽ xử lý phần đó ở cuối
...
class DABModule(nn.Module):
    ...
    def __init__(self, nIn, d=1, kSize=3, dkSize=3):
        ...
        # Thêm ECA Attention vào cuối block
    ...
    def forward(self, input):
        ...
        # Nhánh 1: Tích chập không đối xứng cơ bản
        br1 = self.dconv3x1(output)
        br1 = self.dconv1x3(br1)
        
        # Nhánh 2: Tích chập không đối xứng mở rộng (Dilated)
        ...
        # Áp dụng ECA trước khi cộng residual
...
class DownSamplingBlock(nn.Module):
    ...
    def __init__(self, nIn, nOut):
        ...
        # Thay vì Conv chuẩn, dùng DSConv để hạ kích thước
...
class LightDABNet(nn.Module):
    def __init__(self, classes=1, block_1=3, block_2=6, ch_b1=64, ch_b2=96):
        """
        ch_b1: Kênh cho Block 1 (Gốc là 64, giữ nguyên)
        ch_b2: Kênh cho Block 2 (Gốc là 128, Ép xuống 96 theo Chiến lược 1)
        """
        super().__init__()
        ...
        self.down_1 = InputInjection(1)  # 1/2
        self.down_2 = InputInjection(2)  # 1/4
        self.down_3 = InputInjection(3)  # 1/8
        ...
        self.bn_prelu_2 = BNPReLU((ch_b1 * 2) + 3) # ch_b1 (output1) + ch_b1 (output1_0) + 3
        ...
        self.classifier = nn.Sequential(
            # Ép kênh từ (96*2+3=195) xuống 64
            nn.Conv2d(concat_channels, 64, kernel_size=1, bias=False),
            nn.BatchNorm2d(64),
            nn.Hardswish(inplace=True), # HardSwish nhẹ và hiệu quả hơn PReLU ở các lớp cuối
            nn.Dropout2d(0.1),
            # Phân loại ra số class
...
def build_model(num_classes=1):
    """
    Khởi tạo mạng LightDABNet đã tối ưu.
    - Ép kênh Block 2 xuống 96 (từ 128)
    - Tích hợp ECA Attention
    - DSConv DownSampling
