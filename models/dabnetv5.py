...
class LiteASPPAttention(nn.Module):
    def __init__(self, ch, num_classes=1):
        ...
        # SỬA LỖI TOÁN HỌC: 1 nhánh gốc (ch) + 4 nhánh pool (4*ch) = 5*ch
        fusion_channels = 5 * ch
        
        # Tái trọng số trên Siêu đặc trưng (Theo đúng Bước 5 trong slide của bạn)
        self.eca = ECA_Module(fusion_channels)
        
        # Classifier Bottleneck Head
        self.classifier = nn.Sequential(
            nn.Conv2d(fusion_channels, ch // 2, kernel_size=1, bias=False), 
            nn.BatchNorm2d(ch // 2),
            nn.Hardswish(inplace=True), # Dùng Hardswish mượt hơn ReLU
            nn.Dropout2d(0.1),
            nn.Conv2d(ch // 2, num_classes, kernel_size=1, bias=True)
        )
        
    def forward(self, x):
        # 1. Parallel Pooling Pyramid
        pool_outputs = [x]  # Nhánh 0: Giữ lại bản đồ đặc trưng gốc
        size = x.size()[2:] # Lấy kích thước H, W hiện tại
        
        for pool in self.pools:
            pool_feat = pool(x)
            # Phóng to lại bằng kích thước gốc để nối
            pool_output = F.interpolate(pool_feat, size=size, mode='bilinear', align_corners=False)
            pool_outputs.append(pool_output)
            
        # 2. Concatenate (Tạo "Siêu đặc trưng" 5*ch kênh)
        feat_fusion = torch.cat(pool_outputs, dim=1)
        
        # 3. ECA Attention (Lọc nhiễu kênh trên Siêu đặc trưng)
        feat_fusion = self.eca(feat_fusion)
...
# =========================================================================
# 5. MẠNG CHÍNH (LIGHT DABNET + LITE-ASPP)
# =========================================================================
class LightDABNet(nn.Module):
    def __init__(self, classes=1, block_1=3, block_2=6, ch_b1=64, ch_b2=96):
        ...
        # Stage 2 (Giữ lại chiến lược Hybrid Dilation chống Gridding)
        ...
        # =========================================================
        # TÍCH HỢP ĐẦU LITE-ASPP CHUẨN Template
        # =========================================================
        concat_channels = (ch_b2 * 2) + 3 # 195 channels
        self.aspp_classifier = LiteASPPAttention(ch=concat_channels, num_classes=classes)
    ...
    def forward(self, input):
        ...
        # Đưa vào bộ phân loại đa tầm nhìn (Lite-ASPP)
        out = self.aspp_classifier(output2_cat)
        
        # Upsample lần cuối ra kích thước ảnh gốc
...
def build_model(num_classes=1):
    """
    Khởi tạo mạng LightDABNet V6.
    (DSConv + Hybrid Dilation + LiteASPPAttention Head)
    """
