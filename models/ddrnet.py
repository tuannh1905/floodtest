...
class DDRNetModel(nn.Module):
    def __init__(self, in_channels=3, num_classes=1, planes=32, spp_planes=128, head_planes=64, augment=False):
        """
        DDRNet (Deep Dual-Resolution Network) for real-time semantic segmentation
        ...
        Args:
            ...
            planes: Base number of feature channels (default: 32)
            spp_planes: Feature channels for SPP module (default: 128)
            head_planes: Feature channels for segmentation head (default: 64)
            augment: Whether to use auxiliary head for training (default: False)
        
        Input size: Flexible (e.g., 512x512, 1024x1024)
        """
        super(DDRNetModel, self).__init__()
...
def build_model(num_classes=1, augment=False):
    return DDRNetModel(num_classes=num_classes, augment=augment)