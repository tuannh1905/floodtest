...
# =========================================================================
# 4. BLOCK LITE-ASPP ATTENTION
# =========================================================================
class LiteASPPAttention(nn.Module):
    def __init__(self, ch_in, ch_pool=20, num_classes=1): # TĂNG KÊNH: ch_pool=16 -> 20
        ...
        concat_channels = 3 * ch_pool # 3 * 20 = 60 kênh
        
        self.classifier = nn.Sequential(
            nn.Conv2d(concat_channels, concat_channels, kernel_size=3, padding=1, groups=concat_channels, bias=False), 
...
# =========================================================================
# 5. MẠNG CHÍNH (PRO DABNET V8)
# =========================================================================
class LightDABNet(nn.Module):
    def __init__(self, classes=1, block_1=3, block_2=6, ch_b1=64, ch_b2=128): # TĂNG KÊNH: ch_b2=96 -> 128
        super().__init__()
        ...
        # Cập nhật số kênh đầu vào cho Classifier theo ch_b2 mới
        concat_channels = (ch_b2 * 2) + 3 # (128 * 2) + 3 = 259 channels
        self.aspp_classifier = LiteASPPAttention(ch_in=concat_channels, ch_pool=20, num_classes=classes)
...
def build_model(num_classes=1):
    """
    Khởi tạo mạng ProDABNet V8 (Băng thông cao - Siêu tối ưu):
    1. DSConv Cốt lõi + Init -> Nén tham số mạnh tay.
    2. Hybrid Dilation [1, 2, 4, 8, 1, 2] -> Bảo toàn chi tiết đô thị.
    3. TĂNG KÊNH: ch_b2=128 -> Tăng sức mạnh trích xuất bối cảnh ở Stage 2.
    4. TĂNG KÊNH: LiteASPP ch_pool=20 -> Mở rộng đường truyền Classifier.
    Tổng tham số dự kiến: ~190k - 200k.
    """
    return LightDABNet(classes=num_classes, block_1=3, block_2=6, ch_b1=64, ch_b2=128)