"""
Build-up Case 3: + Pooling Attention  (TripleBranch đầy đủ)
Delta vs Case 2:
- EncoderBlock: thêm Branch 3 (AdaptiveAvgPool -> DWConv -> Sigmoid -> scale input)
- Concat [B1, B2, B3] -> channel_adjust (3*in_c -> out_c)
- Bottleneck, Decoder, Output: giữ nguyên
Đây là TripleBranchEncoderBlock từ encoder.py gốc.
"""
...
# ── NEW: Triple Branch Encoder Block (B1 + B2 + B3) ──────────
class TripleBranchEncoderBlock(nn.Module):
    """
    Branch 1: AxialDW -> PW -> BN -> ReLU
    Branch 2: AxialDW -> PW -> BN -> ReLU -> DualVSSBlock
    Branch 3: AdaptiveAvgPool -> DWConv -> Sigmoid  (channel attention)
              output = input * attention
    Concat [B1, B2, B3] -> channel_adjust (3*in_c -> out_c) -> skip
    MaxPool -> x
    """
...
class UNet_TripleBranch(nn.Module):
    def __init__(self, in_channels=3, num_classes=1):
        ...
        # Encoder: Triple Branch (B1 + B2 + B3)
        ...
        # Bottleneck: giữ nguyên DoubleConv
